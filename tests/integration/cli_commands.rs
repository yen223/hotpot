use super::{TestContext, run_hotpot_command, run_hotpot_with_input, assert_totp_valid, file_contains_account, get_account_count};
use std::fs;
...
fn test_code_command_with_existing_account() {
    ...
    let output = run_hotpot_command(&[
        "--file", ctx.file_path().to_str().unwrap(),
        "code", "github"
    ]);
...
fn test_code_command_with_nonexistent_account() {
    ...
    assert!(!output.status.success(), "Command should fail for nonexistent account");
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("not found") || stderr.contains("Account"), "Should indicate account not found");
}
...
fn test_add_command_creates_new_file() {
    ...
    assert!(ctx.file_path().exists() || !output.status.success(), 
...
fn test_delete_command_removes_account() {
    ...
    if output.status.success() {
        ...
        assert!(!file_contains_account(ctx.file_path(), "github"), "GitHub account should be gone");
    }
}
...
fn test_delete_command_with_no_confirmation() {
    ...
    if output.status.success() {
        ...
        assert!(final_count <= initial_count, "Account count should not increase");
    }
}
...
fn test_export_qr_nonexistent_account() {
    ...
    let output = run_hotpot_command(&[
        "--file", ctx.file_path().to_str().unwrap(),
        "export-qr", "--name", "nonexistent"
    ]);
    
    assert!(!output.status.success(), "Export QR should fail for nonexistent account");
}
...
fn test_multiple_accounts_in_same_file() {
    ...
    let github_code = String::from_utf8_lossy(&github_output.stdout).trim().to_string();
    let google_code = String::from_utf8_lossy(&google_output.stdout).trim().to_string();
...
fn test_invalid_json_file() {
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.to_lowercase().contains("json") || stderr.contains("parse"), 
