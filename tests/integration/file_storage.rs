use super::{TestContext, run_hotpot_command, get_account_count};
use std::fs;
...
fn test_file_creation_with_proper_permissions() {
    ...
    let _output = run_hotpot_command(&[
        ...
        "code", "nonexistent" // This will fail but should create the file
    ]);
...
fn test_parent_directory_creation() {
    ...
    let nested_path = temp_dir.path().join("nested").join("deep").join("accounts.json");
    
    let _output = run_hotpot_command(&[
        "--file", nested_path.to_str().unwrap(),
        "code", "nonexistent"
    ]);
...
fn test_empty_file_handling() {
    ...
    assert!(!output.status.success(), "Command should fail with empty account list");
}
...
fn test_file_with_malformed_json() {
    ...
    assert!(!output.status.success(), "Command should fail with malformed JSON");
}
...
fn test_file_with_missing_required_fields() {
  ...
  "accounts": [
    ...
    assert!(!output.status.success(), "Command should fail when secret is missing");
}
...
fn test_concurrent_file_access() {
    ...
    assert!(success_count > 0, "At least one concurrent access should succeed");
}
...
fn test_file_persistence_across_commands() {
    ...
    for _ in 0..3 {
        ...
        if output.status.success() {
            ...
            assert_eq!(get_account_count(ctx.file_path()), initial_count,
...
fn test_large_file_handling() {
    ...
    assert!(output.status.success(), "Should handle large files correctly");
}
...
fn test_special_characters_in_account_names() {
  ...
  "accounts": [
    ...
    let output2 = run_hotpot_command(&[
        "--file", ctx.file_path().to_str().unwrap(),
        "code", "test-account_123"
    ]);
    
    assert!(output1.status.success(), "Should handle email-like account names");
    assert!(output2.status.success(), "Should handle accounts with special characters");
}